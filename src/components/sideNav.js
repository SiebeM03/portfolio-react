...
const SideNav = ({ currentPath, setCurrentPath }) => {
  ...
  return (
      <nav
          className="hidden lg:flex side-nav h-full items-center before:bg-color-gray before:opacity-35 before:z-10 before:h-7/10 before:max-h-side-nav">
...
const NavItem = ({ page, currentPath, setCurrentPath }) => {
  ...
  return (
      <li onClick={ () => setCurrentPath(page.path) } onTouchEnd={ () => setCurrentPath(page.path) }
