const Header = ({ setCurrentPath, openOuterNav }) => {
  return (
      <header className="flex w-full h-[70px] items-center justify-between z-10">
        <button onClick={ () => setCurrentPath("/") } className="pl-2.5 text-base font-bold uppercase">Siebe Michiels
        </button>
        <div about="header--nav-toggle" onClick={ openOuterNav } className="flex w-12 h-12 flex-col items-center justify-center cursor-pointer
